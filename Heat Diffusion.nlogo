...
to-report heat-diffusivity
  ...
  ;; the approximation is only valid within a certain range
  ;; of time-steps and this range changes depending upon the value of alpha.
  report 0.25 * ((alpha * specific-heat) / thickness-of-layer )
end

;; Sets the material
to update-alpha
  ifelse (material-type = "gold")
  [ set alpha 0.127
    set specific-heat 0.125]
  [
    ifelse (material-type = "copper")
    [ set alpha 0.115
      set specific-heat 0.376 ]
    [
      ifelse (material-type = "iron")
      [ set alpha 0.2034
        set specific-heat 0.460 ]
      [
        ifelse (material-type = "aluminum")
        [ set alpha 0.9711
          set specific-heat 0.921]
        [
          ifelse (material-type = "silver")
          [ set alpha 0.1700
            set specific-heat 0.238]
          [
           ifelse (material-type = "silicon")
           [ set alpha 0.8300
             set specific-heat 0.711]
           [ user-message "Choose your own value for alpha and specific heat!" ]
        ]
      ]
    ]
  ]
...
to remove-temp
  ifelse (temp-edge-remove = "right-temp")
  [ set right-temp 0 ]
  [
    ifelse (temp-edge-remove = "left-temp")
    [ set left-temp 0 ]
    [
      ifelse (temp-edge-remove = "top-temp")
      [ set top-temp 0 ]
      [
        ifelse (temp-edge-remove = "bottom-temp")
        [ set bottom-temp 0 ]
...
208
286
Update Alpha and Specific heat
update-alpha
NIL
1
T
...
material-type
material-type
"gold" "copper" "iron" "aluminum" "silver" "silicon" "other"
0
...
315
188
Choose a material and\nthen press UPDATE ALPHA and SPECIFIC HEAT\nbased on that material,\nor set ALPHA AND SPECIFIC HEAT directly\nfrom the slider.
11
0.0
1
...
temp-edge-remove
temp-edge-remove
"right-temp" "left-temp" "bottom-temp" "top-temp"
1
...
327
407
Choose which edge to have thermal insulation\n
11
0.0
1
